...
/**
 * @title Verifier
 * @notice Verifier contract which stores commitments to the seller's venmoID and verifies proofs of payment.
 * @dev The contract SingleOutputVerifier performs the actual elliptic curve operations to verify the proof. Verifier stores
 * the commitments to the seller's venmoID and verifies additional data about the venmo transaction which gets commited 
 * to in pubSignals[0].
 */
contract Verifier is IVerifier, ReentrancyGuard {
    ...
    /**
     * @dev Maps the address of a seller to a salted poseidon hash of their Venmo ID. Key value pairs are immutable 
     * once set.
     */
    mapping(address => bytes32) public venmoIDHashes; 
   

    /**
     * @dev Each index contains a sellOrderKey. 'Counters' are commited to in the proof, and each counter is used to
     * index the array to get the sellOrderKey for a particular proof/Venmo transaction. There can be multiple counters
     ...
     * (a unique identifier per venmo transaction), a sell order index, and a claim index to a struct showing who the prover 
     ...
     * by a 10 digit decimal number in the Venmo message.
     */
     ...
     * to be able to verify its proof of payment. This allows the claim to be referenced by a 6 digit decimal number in the Venmo
    ...
    /**
     * @notice Called by the seller to commit to their Venmo ID. 
     * @dev Proofs of payments to a given seller are verified against this commitment.
     */
    function onboardSeller(address seller, bytes32 venmoIDHash, bool business) external nonReentrant {
        ...
        require(venmoIDHash != 0 && venmoIDHashes[seller] == 0, "2");

        bytes32 hashValue = venmoIDHash;
            
        // Set the last bit of venmoIDHash to 1 if business is true, or 0 if false
        if (business) {
            // Set the last bit to 1
            venmoIDHash = bytes32(uint256(venmoIDHash) | 1);
        } else {
            // Set the last bit to 0
            venmoIDHash = bytes32(uint256(venmoIDHash) & ~uint256(1));
        }

        venmoIDHashes[seller] = venmoIDHash;

        emit OnboardSeller(seller, hashValue, business);
    }
    ...
    function decodeManually(bytes memory hashInputs) public pure returns (HashInputs memory hashStruct) {
        require(hashInputs.length == 128, "69");
        ...
        // After modHash, the next 32 bytes are venmoIDHash which are again copied into the struct (bytes at indices 64-95).
        assembly {
            temp := mload(add(hashInputs, 64))
        }
        hashStruct.venmoIDHash = temp;

        // After venmoIDHash, the next 32 bytes are Nullifier which are again copied into the struct (bytes at indices 96-127).
        assembly {
    ...
    function verifyProof(bytes calldata proofData, IOrderbook.SellOrder calldata sellOrder, bytes calldata hashInputs) external nonReentrant {
        ...
        {
            /**
             * The commitment to the venmoIDHash is a poseidon hash of 2 field elements, one is the ascii representation of the 
             * seller's venmoID packed into a single field element and the other is a salt. This commitment is intended to be
             * perfectly binding.
             */
            // If the account is a personal account
            if (uint256(hashStruct.venmoIDHash) & 1 == 0) {
                require(hashStruct.venmoIDHash == venmoIDHashes[sellOrder.seller], "5");
            } else {
                require(uint256(hashStruct.venmoIDHash) >> 1 == uint256(venmoIDHashes[sellOrder.seller]) >> 1, "5");
            }
        }

        // Use the venmo message to derive the sellOrderId and claimId
        (uint64 sellOrderId, uint256 claimId) = unpackClaimKey(hashStruct.ClaimKey);
        
        {
            // Verify that the buyer, when sending the Venmo, committed to sellOrderKey and thus SellOrder
            require(sellOrderCtrs[sellOrderId] == sellOrderKey, "6");

            //check enough money was venmo-d
        ...
        {
            /**
             * Use the DNS contract to check that the commitment to  Venmo and Amazon's DNS keys in DNS is equivalent to the
        ...
        /// We assume a bijection between Venmo transactions and verifiedKeys
        ...
        emit ProofVerified(hashStruct.Nullifier, sellOrderId, claimId, hashStruct.Prover);
    }
 ...
 * @notice VeriGroth16Verifierfier contract verifies a groth16 proof with a single output.
 */
